...
export const config = {
  ...
  authDatabaseUrl: process.env.AUTH_DATABASE_URL ?? (process.env.NODE_ENV === "production" ? "" : databaseUrl()),
  complianceDatabaseUrl: process.env.COMPLIANCE_DATABASE_URL ?? (process.env.NODE_ENV === "production" ? "" : databaseUrl()),
  harukiDatabaseUrl: process.env.HARUKI_DATABASE_URL ?? "",
