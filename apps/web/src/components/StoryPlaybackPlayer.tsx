...
export function StoryPlaybackPlayer({ playback }: { playback: StoryPlaybackContext | null }) {
  ...
  return <section className={`story-player story-player-v3 ${textMode ? "text-mode" : ""}`}>
    ...
    <article className="story-player-controls">
      ...
      {(playback?.unavailableReason || warnings.length > 0) && <details className="story-warning-panel"><summary>资源状态与诊断</summary>{playback?.unavailableReason && <p>{playback.unavailableReason}</p>}{warnings.slice(0, 12).map((warning, index) => <p key={`${warning}-${index}`}>{warning}</p>)}<div className="story-diagnostics"><span>Action {actions.length}</span><span>模型 {playback?.live2dModels?.length ?? 0}</span><span>延迟资源 {playback?.deferredAssets?.length ?? 0}</span><span>未支持 {playback?.unsupportedActions?.length ?? 0}</span></div></details>}
    </article>
  </section>;
}
