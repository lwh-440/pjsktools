...
export function StoryPlaybackPlayer({ playback }: { playback: StoryPlaybackContext | null }) {
  ...
  return <section className="story-player story-player-v2">
    <article className={`story-stage ${overlay.shakeScreen ? "is-shaking" : ""}`}>
      ...
      <div className={`story-dialog-box ${overlay.shakeWindow ? "is-shaking" : ""}`}><strong>{overlay.speaker || current?.windowDisplayName || "Story"}</strong><p>{overlay.body || current?.body || "播放后显示真实台词。"}</p></div>
    ...
    <article className="panel story-player-controls">
      <div className="story-player-title"><div><strong>{playback?.scenarioInfo?.episodeTitle ?? playback?.scenarioInfo?.scenarioId ?? "故事播放器"}</strong><small>{playback?.playbackVersion ?? "story-live2d-v2-reference"} / {runtimeStatus}</small></div><span>{actions.length ? `${step + 1} / ${actions.length}` : "无可播放 action"}</span></div>
      ...
      <details><summary>运行时与资源诊断</summary><div className="story-diagnostics"><span>Action {String(playback?.playbackDiagnostics?.actionCount ?? actions.length)}</span><span>模型 {playback?.live2dModels?.length ?? 0}</span><span>媒体 {playback?.mediaAssets?.length ?? 0}</span><span>未支持 {playback?.unsupportedActions?.length ?? 0}</span></div><div className="result-tags"><span>Action 状态</span>{Object.entries(playback?.actionSupport?.status ?? {}).slice(0, 30).map(([key, value]) => <code key={key}>{key}:{value}</code>)}</div><div className="result-tags"><span>运行时</span>{(playback?.runtimeRequirements ?? []).map((item) => <code key={item}>{item}</code>)}</div><div className="result-tags"><span>预加载</span><code>{progress.stage}:{progress.status}</code><code>{progress.completed}/{progress.total}</code></div></details>
    </article>
  </section>;
}
