...
export type RankingPlayerDetail = RankingEntry & {
  ...
  churnUpdatedAt?: string; observedPtUpdates?: number; traceCompleteness?: "complete" | "partial" | "unavailable";
};
...
export function RankingDetailPanel({ detail, mode, leaderCardFallbackUrl, onModeChange }: { detail: RankingPlayerDetail; mode: "player" | "line"; leaderCardFallbackUrl?: string; onModeChange: (mode: "player" | "line") => void }) {
  ...
  return <div className="focus-detail live-detail">
    <section className="live-detail-card">
      ...
      <div className="live-player-card"><div className="live-player-main">{mode === "player" ? <ArtImage src={detail.leaderCardImageUrl} srcCandidates={leaderCandidates} label={`${playerName} 当前队长`} variant="avatar" eager /> : <div className="art-fallback avatar"><span>T{detail.rank}</span></div>}<div><span className="focus-player-name">{mode === "line" ? `T${detail.rank} 档线` : playerName}</span><div className="live-rank-line"><strong>{mode === "line" ? `T${detail.rank}` : `#${detail.rank}`}</strong><b>{formatNumber(currentScore)} pt</b></div><small>采集时间 {formatTime(detail.timestamp ?? activeTrace.at(-1)?.timestamp)}</small>{mode === "player" && <small>队长卡 {detail.leaderCardId ?? detail.cardId ?? "-"} / Lv.{detail.leaderCardLevel ?? detail.cardLevel ?? "-"} / Master Rank {detail.leaderCardMasterRank ?? detail.cardMasterRank ?? "-"}</small>}</div></div><div className="honor-strip"><span>{mode === "player" ? detail.profileWord || "暂无公开签名" : `T${detail.rank} 档线`}</span><span>轨迹采集间隔 {typeof detail.intervalSeconds === "number" ? `${Math.round(detail.intervalSeconds / 60)} 分钟` : "-"}</span><span>{hasChurn ? `近 1H 周回 ${detail.churn1h}` : `近 1H PT 更新 ${detail.observedPtUpdates ?? "—"}`}</span></div></div>
      <div className="live-metric-grid"><div><span>{mode === "player" ? "玩家时速" : "档线时速"}</span><strong className="positive">{typeof growth === "number" ? `+${formatNumber(growth)} pt/h` : "-"}</strong></div><div><span>{hasChurn ? "近 1H 周回" : "近 1H PT 更新次数"}</span><strong>{hasChurn ? detail.churn1h : detail.observedPtUpdates ?? "—"}</strong><small>{hasChurn ? `20分钟×3：${(detail.churn20min ?? 0) * 3}` : detail.traceCompleteness === "complete" ? "采样变化次数，不能代表实际周回场次" : "轨迹数据不足，无法计算"}</small></div><div><span>{hasChurn ? "48H 周回" : "相邻玩家"}</span><strong>{hasChurn ? detail.churn48h : detail.next ? `#${detail.next.rank}` : "-"}</strong><small>{hasChurn ? detail.churnStatus : detail.next ? `${formatNumber(detail.next.score)} pt` : "暂无"}</small></div></div>
    ...
    {hasChurn && (detail.parkingPeriods ?? []).length > 0 && <details className="calculation-details focus-section parking-periods-details"><summary>停车时段（{detail.parkingPeriods!.length} 段）</summary><div className="parking-period-list">{detail.parkingPeriods!.slice().reverse().map((period, index) => <p key={`${period.startTime ?? period.sinceMs ?? "unknown"}-${period.endTime ?? "open"}-${index}`}>{formatParkingPeriod(period)}</p>)}</div></details>}
    <details className="calculation-details focus-section"><summary>查看轨迹采样记录（{activeTrace.length} 条）</summary><div className="trace-table">{activeTrace.slice(-24).map((point) => <div key={`${point.timestamp}-${point.rank}-${point.score}`}><span>{formatTime(point.timestamp)}</span><strong>#{point.rank}</strong><b>{formatNumber(point.score)} pt</b></div>)}{!activeTrace.length && <p className="empty-state">暂无采样记录。</p>}</div></details>
  </div>;
}
