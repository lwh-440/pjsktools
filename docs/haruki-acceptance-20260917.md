...
- 本增量关闭下文旧“预测线摘要6.92h”疑点；上游长期停更时forecast按最新样本回溯、history按当前时间过滤的不同定义尚不在本修复范围。排名Haruki子接口迁移仍未部署，不以本窗口修复替代其验收。
...
| 缺图与上游证据 | 最终页面批次25项缺图：CN服装20、KR称号4、CN素材1；CN前十件服装及称号/素材原有路径404保留，新增十件是页面缺图 | 完整ID见上方批次专项；261011/261021尚未到2026-09-30发布时间。不隐藏名称、不跨区替图，也不将新页面缺图扩大为所有上游路径不存在 |
