...
| Web P3 | 实现冻结；独立最终代码审查通过；待发布提交 | 22 项测试、build、diffcheck 通过；360/768/1440px 无横向溢出；排名分数与增长右对齐并使用 tabular-nums，Tab 聚焦排名行显示约 3px 焦点轮廓；缺失目标档线就地提示且不按 0 规划；手填每局 25000 时本次每局收益显示 25000，主要结果与风险可见，JSON/采样技术信息默认折叠且置信度中文；720px 高视口中 y1953 离屏图片不请求、接近后请求；503 后同 URL 重挂载恢复；模拟缓存候选 onError 后真实备用图片加载成功；eager 图片 503 后离开 600px 观察区再进入自动恢复，无需重挂载，容器高度保持 67.36px；200% 缩放未测 | 本地检查与审查完成；GitHub/main、服务器待发布 |
