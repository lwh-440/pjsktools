...
| --- | --- | --- | --- |
| Web P1 | 独立代码审查通过；待发布提交 | build、16 项测试、diffcheck 通过；390px 菜单收拢/跳转，360/700/768/1440px 无横向溢出及断点导航；详情即时加载、慢请求关闭后不重开；Tab 焦点循环、Esc 返回触发卡；卡池→卡牌嵌套仅关闭子层并返回焦点、背景滚动锁保持；零结果、503 保留缓存、重试加载及恢复；工具连续输入稳定、真实推荐图文结果均已实际验收 | 本地检查与审查完成；GitHub/main、服务器待发布 |
| Web P2 | 待完成 | 待完成 | 待完成 |
| Web P3 | 待完成 | 待完成 | 待完成 |
